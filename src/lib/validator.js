...
class Validator {
    ...
    static validator = {
         ...
         */
        required:
            (expected, message = "Value is required") =>
            (value) =>
         ...
         */
        minLength:
            (expected, message = `Minimum length is ${expected} characters`) =>
            (value) =>
         ...
         */
        maxLength:
            (expected, message = `Maximum length is ${expected} characters`) =>
            (value) =>
                (typeof value === "string" && value.length <= expected) || message,
         ...
         */
        pattern:
            (expected, message = "Value does not match the required pattern") =>
            (value) =>
         ...
         */
        min:
            (expected, message = `Value must be at least ${expected}`) =>
            (value) =>
         ...
         */
        max:
            (expected, message = `Value must be at most ${expected}`) =>
            (value) =>
                (typeof value === "number" && value <= expected) || message,
    };
    ...
    static validate(schema, data) {
        ...
        for (const name in schema) {
            ...
            for (const property in properties) {
                ...
                const method = this.converter[property];
                if (method) {
            ...
            for (const property in properties) {
                ...
                const method = this.validator[property];
                if (method) {
